...
2) Banned imports/strings scan (no network/unsafe modules)
3) Compile gate
4) Content truth: `SUBMISSION_BLOB_SHA256.txt` uses git blob bytes (`git show HEAD:<path>`) for:
   - solver.py
...
Properties:
- sources bytes from git blobs (CRLF-proof)
- fixed entry order
